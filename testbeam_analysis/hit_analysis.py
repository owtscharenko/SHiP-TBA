...
def cluster_hits_niko(input_hits_file, output_cluster_file=None, input_disabled_pixel_mask_file=None, input_noisy_pixel_mask_file=None, min_hit_charge=0, max_hit_charge=None, column_cluster_distance=1, row_cluster_distance=1, frame_cluster_distance=1, dut_name=None, plot=True, chunk_size=1000000):
    '''Clusters the hits in the data file containing the hit table.
    ...
    def calc_cluster_dimensions(hits, clusters, cluster_size,
        ...
        clusters[cluster_index].err_cols = max_col - min_col + 1
        clusters[cluster_index].err_rows = max_row - min_row + 1   
        clusters[cluster_index]['trigger_time_stamp'] = hits[seed_hit_index]['trigger_time_stamp']
...
#             clusters[cluster_hit_indices[i]]['trigger_time_stamp'] = hits[cluster_index]['trigger_time_stamp']
    ...
    clz.set_hit_fields({
                        'trigger_time_stamp': 'trigger_time_stamp'}) # 'relative_BCID': 'frame','tot': 'charge',
    clz.set_hit_dtype([
                       ('trigger_time_stamp', np.uint64)]) # ('column', np.uint16),('tot', np.uint8),
    ...
    def cluster_func(hits, clz, calc_cluster_dimensions):
        ...
        clz.set_hit_fields({
                            'trigger_time_stamp': 'trigger_time_stamp'}) # 'relative_BCID': 'frame','charge': 'charge',
        clz.set_hit_dtype([
                           ('trigger_time_stamp', np.uint64)]) #('column', np.uint16), ('charge', np.uint8),
